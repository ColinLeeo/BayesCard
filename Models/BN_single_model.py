...
from Models.tools import categorical_qcut, discretize_series
...
class BN_Single():
    ...
    def __init__(self, table_name, meta_info=None, method='Pome', debug=True):
        ...
        self.domain = dict()
        self.fanouts = dict()
        self.fanout_indicator = dict()
        self.fanout_sum = dict()
        self.max_value = dict()
        self.method = method
        self.model = None
        self.structure = None
        ...
        for col in table.columns:
            if col in ignore_cols:
                ...
                if col in self.fanout_attr_inverse:
                    ...
                    f = 1
                self.fanout_indicator[col] = f
                table[col], self.n_in_bin[col], self.bin_width[col], self.encoding[col], self.mapping[col], \
                    self.domain[col], self.fanouts[col], self.fanout_sum[col] = discretize_series(
                    table[col],
                    n_mcv=n_mcv,
                    n_bins=n_bins,
